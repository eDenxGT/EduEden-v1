...
import { fetchCourseProgressByStudentId, updateCourseProgressByStudentId } from "../../../store/thunks/courseThunks";
...
const CoursePlayer = () => {
	...
	const completedLectures = courseProgress?.progress?.filter((lecture)=> lecture?.status === "completed").length || 0;
	...
	const handleProgress = (state) => {
		...
		if(state.played > 0.90  && (state.playedSeconds >= playerState.duration-.1) ){
			handleVideoComplete()
		}
	};
	...
	const handleVideoComplete = async () => {
		...
		try {
			...
			dispatch(fetchCourseProgressByStudentId({ course_id, student_id: studentData?.user_id }));
		} catch (error) {
	...
	useEffect(()=>{
		setCurrentLecture(lectures?.find(
			(lecture, index) => index === completedLectures
		))
			...
			<div
				...
				<div className="flex items-center space-x-4">
					<Button
						...
						} min-w-fit border border-orange-200 hover:bg-opacity-80`}
					/>
					<Button
						text="Next Lecture"
					...
					<div className="overflow-auto h-[calc(100vh-12rem)]">
						{lectures?.slice(0,-2)?.map((lecture, index) => (
							<div
								...
								{courseProgress?.progress?.find(item=> (item?.lecture_id === lecture?.lecture_id) && item?.status === "completed") && (
